...
# Setup Device - Forced CPU for Cloud Stability
device = torch.device('cpu') 

# =====================================================
# PATH HELPER (Solves FileNotFoundError on Cloud)
# =====================================================
# Finds the root folder 'multi-scale-pathogenic...' regardless of environment
...
m_col1.metric("🌟 FINAL RISK", f"{final_ensemble_prob:.3f}")
m_col2.metric("📊 ML Ensemble", f"{ml_ensemble_prob:.3f}")
...
i_cols[-1].metric("PathoGAT", f"{gat_prob:.3f}")

st.markdown("---")
col_left, col_right = st.columns(2)
with col_left:
    st.subheader("🎯 Risk Gauge")
    fig_gauge = go.Figure(go.Indicator(mode="gauge+number", value=final_ensemble_prob, gauge={'axis': {'range': [0, 1]}, 'bar': {'color': "red" if final_ensemble_prob > 0.5 else "green"}}))
...
with col_right:
    ...
    fig_bar = go.Figure(go.Bar(x=all_probs[::-1], y=all_names[::-1], orientation='h', marker_color=colors[::-1], text=[f"{p:.3f}" for p in all_probs[::-1]], textposition='auto'))
    fig_bar.add_vline(x=0.5, line_dash="dash", line_color="red")
    fig_bar.update_layout(xaxis=dict(range=[0, 1]), height=350, margin=dict(l=0, r=0, t=30, b=0))
    st.plotly_chart(fig_bar, width='stretch')

st.markdown("---")
st.subheader("🌐 Interactive Protein Network")
local_edges = edges_df[(edges_df["gene1"] == selected_gene) | (edges_df["gene2"] == selected_gene)].head(50)
...
if not local_edges.empty:
    ...
    for gene in genes_in_network:
        net.add_node(gene, label=gene, color="#ff4b4b" if gene == selected_gene else "#1f77b4", size=30 if gene == selected_gene else 15)
    for _, r in local_edges.iterrows():
        net.add_edge(r['gene1'], r['gene2'], color="#888888")
    with tempfile.NamedTemporaryFile(delete=False, suffix=".html") as tmp:
        net.save_graph(tmp.name)
        components.html(open(tmp.name, 'r', encoding='utf-8').read(), height=515)
else:
    st.info("No interactions found for this gene.")
